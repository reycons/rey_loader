...
function GetDate($Node){
    Switch ($Node.LookbackPeriod){
	  ...
	  'month' {$LoadDate = (Get-Date).AddMonths($Node.Lookback)}
...
#write-host "password:" $Password
    ...
    if($ServerInfo.UsePrivateKey -eq 'True'){$Args = $ServerInfo.HostName + " -batch -l " + $ServerInfo.UserName + " -i " + $ServerInfo.PrivateKey + " -b " + $CommandFile}
    else {$Args = $ServerInfo.HostName + " -batch -l " + $ServerInfo.UserName + " -pw " + $password + " -b " + $CommandFile}
#write-host "Args:" $Args
#write-host $ServerInfo.UsePrivateKey
...
#            $FileCharacteristics += $FileDate.toString('yyyy-MM-dd')
            $FileCharacteristics += $DateString
            $FileCharacteristics += "|"
...
Function ConvertCSV(){
    foreach ($Folder in $Config.Configuration.ConvertDelimiter.Folder){
        write-host "Converting Folder:" $Folder.Source
        $Path = $Folder.Source
        $Files = Get-ChildItem -Path $Folder.Source -Include $Folder.FileExtension   
        foreach($File in $Files){
            $NewFileName = $File.BaseName + $Folder.NewFileExtension
            $NewFileName = join-Path $File.DirectoryName $NewFileName
            $NewDelimiter = [char]0x01
            write-host "Converting File:" $File "To" $NewFileName
            Import-CSV $File | Export-CSV -Delimiter $NewDelimiter -Path $NewFileName -NoTypeInformation
            (Get-Content -Path $NewFileName) | ForEach-Object {$_ -Replace '"', ''} | Set-Content -Path $NewFileName
            $NewPath = Join-Path $Folder.MoveTo -ChildPath $File.Name
